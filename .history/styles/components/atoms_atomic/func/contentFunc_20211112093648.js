...
export async function getAllContent(trainingId) {
  const res = await fetch(
    new URL(`${RESTAPI_URL}api/list-content/?training_id=${trainingId}`)
  );
  const contents = await res.json();
  const filteredContents = contents.sort(
    (a, b) => new Date(b.id) - new Date(a.id)
...
export async function getSelectedContentByTrainingId(trainingId) {
  const res = await fetch(
